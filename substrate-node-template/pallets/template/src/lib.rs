...
pub mod pallet {
	...
	#[pallet::storage]
	pub type Number<T:Config> = StorageMap<
	_,
	Blake2_128Concat,
	T::AccountId,
	u32,
	...
	pub enum Event<T: Config> {
		...
		//define an event name to remove a number from accountid
		SomethingRemove(T::AccountId),
	...
	impl<T: Config> Pallet<T> {
		...
		// write a new function for remove number
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1))]
		pub fn remove_number(origin: OriginFor<T>) -> DispatchResult {
			...
			Self::deposit_event(Event::SomethingRemove(who));
			// Return a successful DispatchResultWithPostInfo
			Ok(())
		}
